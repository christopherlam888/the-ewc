...
class GlossaryInstrumentedTest {
    ...
    fun check_random_term_details() {
        ...
        val idx = (0 until glossarySelected.size).random()
        onView(withId(R.id.glossary_recycler_view))
          .perform(RecyclerViewActions.actionOnItemAtPosition<RecyclerView.ViewHolder>(idx, click()))
    ...
    fun check_all_filter() {
        ...
        try {
            onView(withId(R.id.glossary_recycler_view))
                .perform(RecyclerViewActions.scrollToPosition<RecyclerView.ViewHolder>(glossarySelected.size-1))
